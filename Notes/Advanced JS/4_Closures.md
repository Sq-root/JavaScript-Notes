...
- When **`a function is defined within another function`**, it captures the variables and scope of its parent function, creating a closure.
...
const closureExample = outerFunction(); // This returns the Closure
...
##### **Solution** : 
- Using `let` instead of `var` ensures that each iteration of the loop gets its own ==separate copy of the variable==. This block-scoping property of `let` means that when the asynchronous function (e.g., `setTimeout`) executes, it captures the value of `i` as it was at the time of that iteration.
- `let` inside a `for` loop **creates a new `i` for every iteration** — it's **not just reassigned**, it's **re-declared per iteration block**.
- The callback function passed to `setTimeout` **forms a closure over that specific `i`**
...
#### 3. Factory Function
- A **function factory** is a function that **returns another function** — often with **customized behavior** using a **closure**.
- Due to Closure ,  **returned function "remembers"** the variables from its parent (outer) function **even after the outer function has finished**.
...
- Here ,`fetchData()` is the outer function and `setTimeout()` takes an **anonymous inner function** (a callback).
- This anonymous function is scheduled to run **after 1 second**.
- Even though the outer function (`fetchData`) has returned , the inner function still "remembers" `url` and `callback`.
